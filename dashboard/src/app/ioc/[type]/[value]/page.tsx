        ...
        <div className={styles.aiSection}>
          ...
          {aiLoading && (
            <div className={styles.aiLoading}>
              <div className="loading-spinner" />
              <p>GPT-4o-mini is analyzing this IOC...</p>
            </div>
          )}
            ...
            <div className={styles.aiResult}>
              <div className={styles.aiGrid}>
                <div className={styles.aiCard}>
                  ...
                  <span className={getSeverityBadgeClass(aiAnalysis.severity as any)}>
                    {aiAnalysis.severity}
                  </span>
                  <p className={styles.aiConfidence}>Confidence: {aiAnalysis.confidence}%</p>
                </div>
                
                <div className={styles.aiCard}>
                  <h4>AI Risk Score</h4>
                  <span className={styles.aiScore}>{aiAnalysis.aiScore}/100</span>
                  <p className={styles.aiCategory}>{aiAnalysis.threatCategory}</p>
                </div>
                
                {aiAnalysis.threatActors.length > 0 && (
                  <div className={styles.aiCard}>
                    <h4>Threat Actors</h4>
                    <div className={styles.tagList}>
                      {aiAnalysis.threatActors.map((actor, idx) => (
                        <span key={idx} className={styles.actorTag}>{actor}</span>
                      ))}
                    </div>
                  </div>
                )}
                
                {aiAnalysis.mitreTechniques.length > 0 && (
                  <div className={styles.aiCard}>
                    <h4>MITRE ATT&CK</h4>
                    <div className={styles.tagList}>
                      {aiAnalysis.mitreTechniques.map((tech, idx) => (
                        <span key={idx} className={styles.mitreTag}>{tech}</span>
                      ))}
                    </div>
                  </div>
                )}
