...
def build_enriched_ioc_document(ioc_docs: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Note: max_source_risk_score (e.g. cyberint severity_score=100) is preserved
    # in source_evidence as display metadata, but does NOT override the AI score.
    # Per PDF spec, source contribution flows through:
    #   - source_quality factor (Trusted=30 + confidence bonus)
    #   - threat_type_severity factor (× AI confidence)
    # The 6-factor formula stands on its own; no floor override.
