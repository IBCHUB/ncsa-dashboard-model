#!/usr/bin/env python3
"""
Fix the 'severity' field in warehouse to match 'ai_severity'.

The old pipeline set severity from adapter (CyberInt severity_score=100 → "critical").
The correct value is in ai_severity. This script bulk-updates severity=ai_severity
using update_by_query for maximum speed.

Usage:
    python scripts/fix_severity_field.py
    python scripts/fix_severity_field.py --dry-run
    python scripts/fix_severity_field.py --warehouse-url http://192.168.100.43:9200
"""
...
def run_update_by_query(url: str, index: str, dry_run: bool = False):
    ...
    query = {
        "script": {
            "source": "ctx._source.severity = ctx._source.ai_severity",
            "lang": "painless",
        },
        "query": {
            "bool": {
    ...
    if dry_run:
        ...
        print(f"[DRY RUN] Would update severity=ai_severity for up to {total:,} docs")
        return

    print(f"Running update_by_query on {url}/{index}...")
    print("This may take several minutes for large indices.")
    t0 = time.time()
...
def main():
    parser = argparse.ArgumentParser(description="Fix severity field to match ai_severity")
